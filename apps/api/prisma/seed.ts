...
async function main() {
  ...
  const grantsData = [
    {
      walletId: wallet.id,
      grantType: 'token' as const,
      grantName: '平台启动额度',
      sourceType: 'park_grant' as const,
      quantityTotal: 10000,
    ...
    {
      walletId: wallet.id,
      grantType: 'model_coupon' as const,
      grantName: '模型体验券',
  ...
  const seededClients: Record<string, { id: string }> = {};
  ...
  // Seed Projects with initial ScopeVersion
  const projectsData = [
    {
      id: 'seed-project-1',
      organizationId: organization.id,
  ...
  // Seed ProfitLedgers for each project
